  ...
  return (
    <form className="grid grid-cols-1 sm:grid-cols-2 gap-5 w-full text-left" onSubmit={submit}>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Nama Lengkap</label>
        <input 
          required 
          value={form.name} 
      ...
      {/* 2. WhatsApp Number */}
      <div className="flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Nomor WhatsApp (Aktif)</label>
        <input 
          required 
          type="tel"
          value={form.phone} 
          ...
          className={`w-full bg-accent-light/35 border px-4 py-3 rounded-[2px] text-xs text-primary focus:border-accent transition-all duration-300 outline-none focus:ring-1 ${
            status === "invalid" 
      ...
      {/* 3. Project Location */}
      <div className="flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Lokasi Proyek / Alamat</label>
        <input 
          required 
          value={form.location} 
      ...
      {/* 4. Type of Need */}
      <div className="flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Jenis Pekerjaan</label>
        <select 
          value={form.need} 
      ...
      {/* 5. House Area */}
      <div className="flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Bagian Ruang Rumah</label>
        <select 
          value={form.area} 
      ...
      {/* 6. Estimated Size */}
      <div className="flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Ukuran Perkiraan (m / cm)</label>
        <input 
          value={form.size} 
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Estimasi Anggaran Proyek (Rp)</label>
        <input 
          value={form.budget} 
      ...
      <div className="sm:col-span-2 flex flex-col gap-1">
        <label className="text-[9px] uppercase font-semibold tracking-luxury text-primary mb-1">Catatan Khusus / Permintaan Kustom</label>
        <textarea 
          value={form.notes} 
          onChange={(event) => update("notes", event.target.value)} 
          placeholder="Ceritakan kebutuhan ruang Anda, pilihan warna favorit, atau kondisi khusus di lokasi..." 
          className="w-full bg-accent-light/35 border border-border-premium hover:border-accent/60 px-4 py-3 rounded-[2px] text-xs text-primary min-h-[100px] focus:border-accent transition-all duration-300 outline-none focus:ring-1 focus:ring-accent/10"
        />
      </div>
      ...
      <div className="sm:col-span-2 flex flex-col gap-3">
        {status === "invalid" && (
          <div className="bg-red-50 border border-red-200 text-red-700 text-xs p-4 rounded-[2px] flex items-center gap-2" role="alert" aria-live="polite">
            <AlertTriangle size={14} className="flex-shrink-0" />
            <span>Format WhatsApp salah. Gunakan 10-15 digit angka yang valid (misal: 085119467138).</span>
          </div>
        )}
        {status === "success" && (
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs p-4 rounded-[2px] flex items-center gap-2" role="alert" aria-live="polite">
            <CheckCircle2 size={14} className="flex-shrink-0" />
            <span>Data pengerjaan proyek berhasil tersimpan! Draf konsultasi WhatsApp akan terbuka secara otomatis.</span>
          </div>
        )}
        {status === "fallback" && (
          <div className="bg-amber-50 border border-amber-200 text-amber-700 text-xs p-4 rounded-[2px] flex items-center gap-2" role="alert" aria-live="polite">
            <CheckCircle2 size={14} className="flex-shrink-0" />
      ...
      <div className="sm:col-span-2 pt-2">
        <button 
          ...
          className="w-full py-4 bg-accent hover:bg-accent-hover text-primary hover:text-primary-dark text-[10px] font-semibold uppercase tracking-luxury rounded-[2px] shadow-sm transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
        >
          {isSubmitting ? (
